...
├── LICENSE
├── README.md
└── openrockcli             Linux x86_64 binary
```

This repository intentionally carries the prebuilt OpenRockCLI binary only. It
does not include the OpenRockCLI source tree, headers, payload sources, or
Makefiles.

## Linux Runtime

OpenRockCLI depends on the runtime `libusb-1.0` and `libudev` libraries. On
Ubuntu, install:
...
Use the bundled binary from the repository root:

```shell
tools/openrockcli/openrockcli --version
```
