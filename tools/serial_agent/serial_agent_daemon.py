#!/usr/bin/env python3
"""Single-owner serial daemon for RK3506 debug.
...
class SerialAgent:
    ...
    def _handle_request(self, req):
        ...
        if action == "write":
            ...
            return {"ok": True}
        if action == "break":
            duration = float(req.get("duration", 0.25))
...
def main():
    ...
    ap.add_argument("--baudrate", type=int, default=1500000)
    ap.add_argument("--unix-sock", default="/tmp/rk3506-serial.sock")
    ap.add_argument("--tcp-host", default="127.0.0.1")
    ap.add_argument("--tcp-port", type=int, default=23333)
    ap.add_argument("--log-file", default="/tmp/rk3506-serial.log")
