# Serial AI Agent 套件 (RK3506)

面向 Linux 串口设备的 AI Agent 工具集，基于 `PySerial + LangChain/CrewAI`，并提供 `pexpect` 交互能力。当前默认适配 **Rockchip RK3506** 平台。
...
- 串口: /dev/ttyUSB0
- 波特率: 1500000 8N1
- 烧录: Loader/Maskrom RockUSB 模式，配合 `tools/openrockcli/openrockcli`
- SDK 路径: 仓库根目录
...
```bash
python3 trae_serial_terminal.py terminal --port /dev/ttyUSB0 --baudrate 1500000
```

纯透传模式（字符级，不等回车，行为更接近 putty）：

```bash
python3 trae_serial_terminal.py terminal-raw --port /dev/ttyUSB0 --baudrate 1500000
```
...
目录内提供预编译 Go 串口终端二进制 `trae_serial_terminal_go`，支持 `scan/io/terminal/terminal-raw`。

示例：

```bash
./trae_serial_terminal_go scan --json
./trae_serial_terminal_go terminal --port /dev/ttyUSB0 --baudrate 1500000
```
...
t = PexpectSerialTerminal("/dev/ttyUSB0", 1500000)
t.open()
t.login("root", "123456")
...
- 默认串口参数是 `1500000 8N1`，并支持 `xonxoff/rtscts/dsrdtr` 流控配置。
